...
export default function PsicoterapiaReportPage() {
  ...
  const [formData, setFormData] = useState<PsicoterapiaFormData>({
    relatorioDaSessao: "",
    presente: true,
  });
  ...
  const fetchSessionData = async () => {
    ...
    try {
      ...
      const psicoterapiaProntuario = foundSession.Prontuario?.find((p: MedicalRecordEntry) => p.id_Tipo === 2);
      if (psicoterapiaProntuario) {
        ...
        router.push(`/dashboard/relatorios/psicoterapia/${sessionId}/edit`);
        return;
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await apiService.createMedicalRecordPsicoterapia(payload, session.token);

      toast.success("Relatório de psicoterapia salvo com sucesso!");
    ...
    <div className="container mx-auto py-6 max-w-4xl">
      <div className="mb-6">
        ...
        <div className="flex items-center gap-2 mb-2">
          ...
          <h1 className="text-3xl font-bold">Preencher Relatório de Psicoterapia</h1>
        </div>
        <p className="text-muted-foreground">
          Preencha as informações sobre a sessão de psicoterapia realizada
        </p>
      </div>
      ...
      {/* Formulário de Psicoterapia */}
      <Card>
        <CardHeader>
          <CardTitle className="text-lg">Relatório de Evolução</CardTitle>
          <CardDescription>
            Preencha os dados referentes à sessão de psicoterapia
          </CardDescription>
        </CardHeader>
        <CardContent>
